...
趣味でセレクトショップや古着屋巡りをするときに、ショップの場所や取り扱いのあるブランドがまとめられたサイトといったものがあまり存在せず、情報を得るためにはショップの情報をInstagramやHPでチェックする必要がありました。<br>
こうしたショップや取り扱いのあるブランドをまとめたプラットフォームがあれば、より効率的にお店巡りができるのではないかと考え、本アプリケーションを作成しました。
...
現状ローカルでの開発環境でしか動作確認をしていないため、今後AWSを用いて本番環境へデプロイしたい。<br>
Dockerを使用しているため、理想としては```ECS（Fargate）```でコンテナデプロイをし、```Terraform```でインフラのコード化を試してみたい。
